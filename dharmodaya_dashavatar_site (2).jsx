...
export default function DharmodayaDashavatarSite() {
  ...
  const gods = [
    ...
    },
    {
      ...
      images: [
        "https://images.unsplash.com/photo-1631125915902-d8abe9225ff2?auto=format&fit=crop&w=900&q=80",
  ...
  const nav = ["Home", "About", "Dashavatar", "Gods", "Events", "Temple Project", "Gallery", "Contact"];

  const activeDetail = useMemo(() => selectedAvatar || selectedGod, [selectedAvatar, selectedGod]);
          ...
          <div className="relative mx-auto grid max-w-7xl gap-12 px-4 py-16 sm:px-6 lg:grid-cols-2 lg:px-8 lg:py-24">
            <div className="flex flex-col justify-center">
              ...
              <h2 className="max-w-2xl text-4xl font-black leading-tight text-stone-900 sm:text-5xl lg:text-6xl">
                A living heritage of <span className="text-orange-700">Dashavatar Balan</span>, devotion, and community service.
              </h2>
            ...
            <div className="mt-10 grid gap-6 lg:grid-cols-3">
              {events.map((event) => (
                <div key={event.title} className="rounded-[1.75rem] border border-white/10 bg-white/5 p-6 backdrop-blur">
                  ...
                  <button className="mt-5 rounded-full border border-orange-300 px-4 py-2 text-sm font-semibold text-orange-200 transition hover:bg-orange-500/10">
                    Event Details
                  </button>
                </div>
              ))}
        ...
        <section id="gallery" className="bg-white py-16">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
              <div>
                <p className="text-sm font-semibold uppercase tracking-[0.25em] text-orange-700">Gallery concept</p>
                <h3 className="mt-3 text-3xl font-black text-stone-900 sm:text-4xl">Visual storytelling for culture, devotion, and progress</h3>
              </div>
              <button className="w-fit rounded-full border border-stone-300 px-5 py-3 text-sm font-semibold text-stone-800 transition hover:border-orange-400 hover:text-orange-700">
                View Full Gallery
              </button>
            </div>
            <div className="mt-10 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              {[
                "Dashavatar performance moments",
                "Jhaki and festival celebrations",
                "Temple construction updates",
                "Community members and devotees",
              ].map((item, index) => (
                <div key={item} className={`group flex min-h-[240px] items-end overflow-hidden rounded-[1.75rem] border border-amber-200 bg-gradient-to-br ${index % 2 === 0 ? "from-orange-100 to-amber-50" : "from-amber-100 to-yellow-50"} p-6 shadow-sm`}>
                  <div>
                    <span className="rounded-full bg-white/80 px-3 py-1 text-xs font-semibold text-orange-700">Gallery</span>
                    <h4 className="mt-3 text-xl font-bold text-stone-900">{item}</h4>
                  </div>
                </div>
              ))}
            </div>
        ...
        <section id="contact" className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
          <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
            ...
            <div className="rounded-[2rem] border border-amber-200 bg-gradient-to-br from-amber-50 to-white p-8 shadow-xl">
              <div className="grid gap-4 sm:grid-cols-2">
                <input className="rounded-2xl border border-amber-200 bg-white px-4 py-3 outline-none ring-0 placeholder:text-stone-400 focus:border-orange-400" placeholder="Your name" />
                <input className="rounded-2xl border border-amber-200 bg-white px-4 py-3 outline-none ring-0 placeholder:text-stone-400 focus:border-orange-400" placeholder="Phone or email" />
                <input className="rounded-2xl border border-amber-200 bg-white px-4 py-3 outline-none ring-0 placeholder:text-stone-400 focus:border-orange-400 sm:col-span-2" placeholder="Subject" />
                <textarea className="min-h-[140px] rounded-2xl border border-amber-200 bg-white px-4 py-3 outline-none ring-0 placeholder:text-stone-400 focus:border-orange-400 sm:col-span-2" placeholder="Write your message, support interest, or event inquiry..." />
